...
/* 🚪 앱의 문.
   ...
   ⚠️ 여기서 `await bootstrap()` 을 «먼저» 하면 안 된다 — 그게 끝나기 전에는 **아무것도 안 그린다.**
      준비가 오래 걸리는 동안 회원이 보는 것은 안드로이드 기본 스플래시뿐이라
      「앱이 고장났다」로 읽힌다. 그래서 그리는 일을 먼저 시작하고, 준비는 그 뒤에 돈다.

   📏 2026-08-29 실측(에뮬레이터에서 인터넷을 끊고 켬) — 숫자는 **에뮬 기준**이라
      실제 폰은 이보다 훨씬 빠르다. 그래도 «어느 쪽이 문제인지»는 갈라 준다:
        · 디버그 판: 첫 화면까지 2분 6초, 15초 지킴이가 찍은 글도 118초 뒤 (디버그는 원래 느리다)
        · 릴리즈 판: 첫 화면까지 34.6초, 지킴이는 **제때(15초)** 울렸다
      즉 「지킴이가 안 돈다」는 디버그 탓이었다. 다만 준비가 끝나야 그리는 구조 자체는
      회원에게 빈 화면을 보이므로, 그리는 일을 먼저 하는 편이 옳다. */
void main() {
...
class _BootAppState extends State<BootApp> {
  ...
  void initState() {
    super.initState();
    /* 첫 그림 뒤로 미뤄 봤지만(addPostFrameCallback) **재 보니 나아지지 않았다** —
       34.6초에서 50.4초로 오히려 늘었다(에뮬레이터, 릴리즈 판, 인터넷 끊고 켬).
       첫 그림이 늦는 건 우리 코드가 아니라 엔진이 서는 데 걸리는 시간이 지배적이다.
       근거 없는 복잡함은 남기지 않는다. */
    _go();
  }
