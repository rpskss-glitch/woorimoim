...
class OwnerGuideCard extends StatefulWidget {
  ...
  static bool get dismissed => Store.i.getInt(_key) == 1;
  static void dismiss() => Store.i.setInt(_key, 1);

  /// 다시 보기 (설정에서 부른다)
  static void reset() => Store.i.setInt(_key, 0);

  /// 지금 이 회원에게 보여야 하는가
  static bool shouldShow() => AppState.i.isOwner && !dismissed;
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    final ok = await confirmSheet(
                      ...
                      '안내서를 닫을까요?',
                      '이 기기에서는 다시 안 떠요.\n'
                          '다시 보시려면 설정 맨 아래 「방장 안내서 다시 보기」를 누르세요.',
                      okLabel: '닫기',
                    );
                    if (!ok) return;
