  ...
  Future<void> _newClub() async {
    ...
    final ok = await confirmSheet(
      ...
      '만든 사람이 방장이 되고, 회원은 이 이름으로 가입 신청을 해요.\n\n'
          ...
          /* ⚠️ 예전 「먼저 만들어 보시고, 회원을 부르기 전에 결제하시면 돼요」는 사실이 아니었다 —
                새 모임은 이용권 전에는 대화·일정·회비·게시판을 새로 쓸 수 없다(규칙 clubPaid).
                방장 안내(owner_guide)와 같은 말로 맞춘다(2026-09-25 조사). */
          '만든 뒤 결제 전에도 회원 승인·설정은 되고, 대화·일정·회비 기록·게시판은 결제한 뒤부터 쓸 수 있어요.',
      okLabel: '모임 만들기',
    );
    if (!ok || !mounted) return;
