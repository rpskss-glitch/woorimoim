    ...
    for (final x in arr) {
      ...
      if (x['kind'] == 'poll') {
        ...
        x['poll'] = {
          ...
          /* ⏳ 마감 시각 — **여기서 빠뜨리면 기한이 통째로 사라진다.**
             이 자리는 투표를 «다시 만들어» 넣는 곳이라, 안 적은 칸은 그대로 없어진다.
             2026-08-30 실제로 그랬다: 「3시간」을 골라 올렸는데 화면에 남은 시간이
             안 떴고, 기한이 지나도 투표가 안 닫혔다.
             ⚠️ 숫자가 아닌 값은 버린다 — 글자가 오면 셈하는 자리에서 터진다. */
          if (p['until'] is num) 'until': (p['until'] as num).toInt(),
        };
      }
