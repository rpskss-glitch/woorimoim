...
/* 💸 지출 기록 창도 — 적은 금액·내용이 «한 번 톡»에 사라지지 않게. (2026-09-26, 70·72회차와 같은 고침) */
void main() {
  tearDown(() {
    Demo.stop();
    AppState.i.setItems([]);
  ...
  testWidgets('금액을 적었으면 바깥을 눌러도 안 닫히고 묻는다', (t) async {
    ...
    await t.pumpAndSettle();
    await t.enterText(find.byType(TextField).first, '셔틀콕 두 통');
    await t.pump();
    await t.tapAt(const Offset(20, 20));
    await t.pumpAndSettle();
    expect(find.text('쓰던 글을 버릴까요?'), findsOneWidget);
    expect(find.text('셔틀콕 두 통'), findsOneWidget);
  });
}
