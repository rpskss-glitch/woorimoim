...
void main() {
  ...
  test('main 은 «기다리지 않는다» — 그리는 일이 먼저다', () {
    /* 준비가 끝나야 그리는 구조면, 그동안 회원이 보는 건 안드로이드 기본 스플래시뿐이라
       「앱이 고장났다」로 읽힌다. 그래서 `runApp` 을 먼저 부르고 준비는 그 뒤에 돈다.

       📏 2026-08-29 에뮬 실측(인터넷 끊고 켬) — 실제 폰은 이보다 훨씬 빠르다:
          디버그 판 2분 6초 · 릴리즈 판 34.6초. 「15초 지킴이가 안 돈다」던 것은
          디버그 탓이었고, 릴리즈에서는 제때 울렸다. 수치는 그렇더라도
          «빈 화면을 보이지 않는다»는 뜻은 그대로 지킨다. */
    final body = bodyOf(src, 'void main(');
