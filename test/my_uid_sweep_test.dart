...
const _known = {
  'board.dart': 1, // 글 지우기 권한 — 서버가 글에 적힌 번호만 보므로 넓히면 헛단추
  /* 사진 지우기 권한 ×2 (한 장 / 여러 장 한꺼번에).
     ⚠️ 여기는 **넓히면 안 된다.** 서버는 사진에 적힌 번호(`by`)만 보므로,
        폰 바꾸기 전 번호까지 「내 것」으로 쳐서 단추를 띄우면 눌러도 거절당한다.
     («보여 주기» 쪽인 반응 세기는 이 파일에서도 Logic.isMe 로 잇는다) */
  'album.dart': 2,
  ...
  'chat.dart': 2, // 지우기 권한 1 + 지금 회원 목록 안에서의 비교 1 (읽음 셈은 2026-09-25 Logic.readCount 로 옮김 — 그쪽은 지금 회원만 본다)
  /* 2026-09-25: post_screen 의 한 자리(댓글 메뉴 «내 댓글인지»)가 빠졌다.
     예전 설명은 «넓히면 옛 내 댓글에 나를 차단이 뜬다»였는데 **거꾸로였다** — 번호를 그대로
     견주니 폰 바꾼 회원의 옛 댓글이 «남의 것»이 되어 차단이 떴고, 누르면 나를 차단했다.
     이제 Moderation.canBlock(폰 바꾸기까지 잇는 «남인가»)로 본다 — chat_moderation_fixes_test 가 지킨다. */
  'members.dart': 2, // 나를 자르거나 나에게 방장을 넘길 수 없다 (목록 안이라 옛 번호가 없다)
  /* 2026-09-25: 같은 폰에서 «다른 사람»이 로그인할 때 새 로그인 번호를 받았는지 확인.
     «방금 바꾸기 전 번호와 같은가»를 묻는 것이라 옛 번호로 넓히면 뜻이 없어진다. */
  'onboarding.dart': 1,
